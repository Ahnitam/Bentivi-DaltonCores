...
public class CameraColorPickerPreview extends TextureView implements TextureView.SurfaceTextureListener,
    ...
    private Camera.Parameters camParametros;
    protected int[] mSelectedColor;
    protected OnColorSelectedListener corSelecionada;
    ...
    public CameraColorPickerPreview(Context context, Camera camera) {
        ...
        camParametros = camera.getParameters();
        if (camParametros.getSupportedFocusModes().contains(
                Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO)) {
            camParametros.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO);
        }
        camera.setParameters(camParametros);
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        if (corSelecionada != null) {
            ...
            corSelecionada.onColorSelected(Color.rgb(mSelectedColor[0], mSelectedColor[1], mSelectedColor[2]));
        }
    }
    ...
    public void setOnColorSelectedListener(OnColorSelectedListener onColorSelectedListener) {
        corSelecionada = onColorSelectedListener;
    }
